...
<header class="sticky top-0 z-40 w-full border-b border-border/60 bg-background/80 backdrop-blur-lg">
  <div class="container flex mx-auto h-16 items-center justify-between gap-3 p-4">
    ...
    <div class="flex items-center gap-3">
      ...
      {:else}
        <a href="/passenger-dashboard" class="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-700 text-white shadow-card" aria-label="Home">
          <Car class="w-5 h-5"/>
        </a>
      {/if}
      <div class="leading-tight">
        <p class="text-sm font-semibold text-foreground">{title}</p>
        <p class="text-xs text-muted-foreground">Passenger · Campus Transport</p>
      </div>
    </div>
