...
<div class="min-h-screen bg-background pb-12">
  ...
  <main class="container mx-auto max-w-2xl space-y-6 px-4 py-6">
    ...
    <section>
      ...
      <h1 class="mt-1 text-2xl font-bold tracking-tight text-foreground">Ready to drive?</h1>
    </section>
    ...
    <section>
      ...
      {#if !$activeRide}
        <div class="rounded-3xl border border-dashed border-border/60 bg-card p-6 text-center space-y-1">
          ...
          <p class="text-xs text-muted-foreground">Browse requests or create an offer to get started.</p>
        </div>
        ...
        <div class="rounded-3xl border border-border/60 bg-card p-5 shadow-card space-y-3">
          ...
          <div class="rounded-2xl border border-border/60 bg-muted/30 px-4 py-3 space-y-1 text-xs text-muted-foreground">
            <div class="flex justify-between">
              <span>Passenger</span>
              <span class="font-medium text-foreground">{$activeRide.passengerName}</span>
            </div>
            <div class="flex justify-between">
            ...
            <div class="flex justify-between">
              <span>Seats</span>
              <span class="font-medium text-foreground">{$activeRide.seats}</span>
            </div>
          </div>
          <div class="flex items-start gap-2.5 rounded-2xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/40 px-4 py-3">
            ...
            <div>
              <p class="text-xs font-semibold text-amber-700 dark:text-amber-400">Waiting for passenger to confirm</p>
              <p class="text-xs text-amber-600/80 dark:text-amber-500/80 mt-0.5">
                {$activeRide.passengerName} will accept or decline your offer shortly.
              </p>
            </div>
          </div>
        </div>
    ...
    <section class="rounded-3xl bg-gradient-soft p-5">
      ...
      <p class="mt-1 text-xs text-muted-foreground">All chats unlock only after a match. Share your trip with friends from the ride page.</p>
    </section>
