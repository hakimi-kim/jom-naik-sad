<script lang="ts">
  ...
  // 1. Use the $state rune for reactivity
  let id = $state('');

  function handleLogin(e: SubmitEvent) {
    e.preventDefault();
    // In a real app, you'd validate 'id' here
    goto('/dashboard');
  }
</script>
...
<div class="flex min-h-screen flex-col items-center justify-center bg-gradient-soft px-4">
  <div class="mx-auto w-full max-w-md animate-fade-in py-12">
    ...
    <div class="mb-8 flex flex-col items-center text-center">
      <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-hero shadow-elevated">
        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M19 17H5a2 2 0 0 1-2-2V9a2 2 0 0 1 2-2h1l2-3h8l2 3h1a2 2 0 0 1 2 2v6a2 2 0 0 1-2 2z"/><circle cx="12" cy="13" r="2"/>
        </svg>
      </div>
    ...
    <div class="rounded-3xl bg-card p-6 shadow-elevated sm:p-8">
      ...
      <p class="mt-1 text-sm text-muted-foreground">Sign in with your student credentials</p>

      <form onsubmit={handleLogin} class="mt-6 space-y-4">
        <div class="space-y-2">
          <label for="id" class="block text-sm font-medium text-foreground">Student ID or Email</label>
          <input
            id="id"
            type="text"
            required
